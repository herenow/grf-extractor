...
function extractStart() {
	...
	var extract_folder = this.output_dir.replace(/\/?$/, '/') // Add trailling slash to folder
	var self = this
		...
		function extract_next() {
			...
			Fs.read(grf.file.fd, buffer, 0, entry.length_aligned, entry.offset + GRF.struct_header.size, function(err, bytesRead, buffer){
				...
				Zlib.inflate(buffer, function(err, buf) {
					if(err) {
						console.error(file, "Failed to inflate", err)
					}
					
					// Create directoy
					if(typeof self.mkdirPaths[path] === 'undefined') {
						Mkdirp(path, function (err) {
							if(err) {
								console.error(path, "Failed to mkdirp.")
							}
